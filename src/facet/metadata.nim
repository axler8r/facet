...
proc resolveFileId*(db: DbConn, root: string, filePath: string, register: bool = false): int =
  ...
  if lstat(full.cstring, info) == 0:
    ...
    if not register:
      ...
      let now = utcNowNs()
      let mtimeNs = int64(info.st_mtim.tv_sec) * 1_000_000_000'i64 + int64(info.st_mtim.tv_nsec)
      if identity.isSome:
        let previous = identity.get
        var canonicalInfo: Stat
        var canonical = rel
        let canonicalFull = root / previous.path
        if lstat(canonicalFull.cstring, canonicalInfo) == 0 and S_ISREG(canonicalInfo.st_mode) and
            cast[uint64](canonicalInfo.st_dev) == device and cast[uint64](canonicalInfo.st_ino) == inode:
          canonical = previous.path
        updateFileRecord(db, previous.id, canonical, info.st_size, mtimeNs, now)
        return previous.id
      return insertFileRecord(db, rel, device, inode, info.st_size, mtimeNs, now, now)
...
proc setAttribute*(db: DbConn, root: string, filePath: string, attributeName: string, rawValue: string) =
  db.transaction:
    ...
    let currentRows = db.all("SELECT value_text, value_integer, value_real, value_boolean FROM attribute_values WHERE file_id = ? AND attribute_id = ?", fileId, def.id)
    let oldValue = if currentRows.len > 0: some(decodeAttributeRow(def, currentRows[0])) else: none(string)
    if oldValue == some(valid):
      return

    db.exec("INSERT INTO attribute_values(file_id, attribute_id, value_text, value_integer, value_real, value_boolean) VALUES(?, ?, ?, ?, ?, ?) ON CONFLICT(file_id, attribute_id) DO UPDATE SET value_text = excluded.value_text, value_integer = excluded.value_integer, value_real = excluded.value_real, value_boolean = excluded.value_boolean", fileId, def.id, if def.kind == "string" or def.kind == "enum": valid else: "", if def.kind == "integer": parseInt(valid) else: 0, if def.kind == "real": parseFloat(valid) else: 0.0, if def.kind == "boolean": (if valid == "true": 1 else: 0) else: 0)
    db.exec("INSERT INTO attribute_history(file_id, attribute_id, old_value, new_value, changed_at) VALUES(?, ?, ?, ?, ?)", fileId, def.id, oldValue, valid, utcNowNs())

proc unsetAttribute*(db: DbConn, root: string, filePath: string, attributeName: string) =
  db.transaction:
    let defOpt = fetchAttributeDef(db, attributeName)
    if defOpt.isNone:
      raise newException(ValueError, "attribute not found: " & attributeName)
    ...
    db.exec("DELETE FROM attribute_values WHERE file_id = ? AND attribute_id = ?", fileId, def.id)
    db.exec("INSERT INTO attribute_history(file_id, attribute_id, old_value, new_value, changed_at) VALUES(?, ?, ?, ?, ?)", fileId, def.id, oldValue, none(string), utcNowNs())

proc fileAttributes*(db: DbConn, fileId: int): seq[tuple[name: string, value: string]] =
...
proc printFileDetails*(db: DbConn, root: string, filePath: string, jsonMode: bool = false): string =
  let fileId = resolveFileId(db, root, filePath)
...
proc getHistory*(db: DbConn, filePath: string, attributeName: string = "", root: string = ""): seq[tuple[attribute: string, oldValue: string, newValue: string, changedAt: int64]] =
  var fileId: int
  if root.len > 0:
